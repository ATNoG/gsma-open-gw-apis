...
class RedisGeofencingSubscriptionInterface(GeofencingSubscriptionInterface):
    ...
    async def create_location_retrieval_subscription(
        ...
        # NOTE: The NEF Emulator in use only supports external id for
        # MonitoringNotifications. This means that we have to use
        # networkAccessIdentifier, altough CAMARA is not meant to support it
        # yet.
        if device.networkAccessIdentifier is None:
            raise ApiException(
