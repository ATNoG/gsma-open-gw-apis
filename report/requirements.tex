...
directly with the system, have requirements that need fulfilling, for example,
...
Thus, in this chapter we will not only introduce the actors who will interact
...
\begin{itemize}
	...
	\item \textbf{\emph{Vertical Clients}}\cite{5g-ppp} - The second actor to
...
are shown in Figure \ref{fig:useCase}.

\begin{figure}[H]
	\centerline{
		\includegraphics[width=15cm]{figs/use_case_diagram.png}
	}
...
\begin{itemize}
	...
	\item{\textbf{Vertical Clients}}
	      ...
	      \begin{enumerate}
		      \item Send/Validate OTPs, to (for example) check the validity of a phone
		            number via SMS.
		      ...
		      \item Check the presence of a device, such as checking whether a particular
...
\begin{itemize}
	\item \textbf{Performance and Latency:}
	      \begin{itemize}
		      \item The system should maintain consistent response times as the
		            user base or data volume increases, on average 100 ms, and on
		            peaks it should not be above 400ms.

		      \item The system must be able to handle an increasing number of
		            requests (e.g. 10 request per second) .
	      \end{itemize}

	\item \textbf{Scalability:}
	      \begin{itemize}
		      \item The system must ensure support for a high volume of requests
		            and connected devices (e.g. 150 devices doing on average 5 requests
		            per minute).
	...
	\item \textbf{Availability and Reliability:}
	      \begin{itemize}
		      ...
		      \item The system must have a high level of reliability (e.g. 99.9999\%
		            reliability).
